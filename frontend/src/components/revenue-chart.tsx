...
type MatriculaAggRow = {
  data_inicio?: string
  valor_pago?: number | null
}

const monthsShort = ['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez'] as const
...
export function RevenueChart({ academiaId }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const matriculas = (await listMatriculasByDataInicio(
        academiaId,
        min
      )) as MatriculaAggRow[]
      const withValor = matriculas.filter(
        (m) => m.valor_pago != null && Number(m.valor_pago) > 0
      )
      ...
      for (let i = 0; i < 6; i++) {
        ...
        const key = `${monthsShort[d.getMonth() as number]}/${d.getFullYear().toString().slice(-2)}`
        grouped[key] = 0
      }

      for (const m of withValor) {
        if (!m.data_inicio) continue
        const date = new Date(`${m.data_inicio}T12:00:00`)
        const key = `${monthsShort[date.getMonth()]}/${date.getFullYear().toString().slice(-2)}`
